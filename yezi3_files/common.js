		...
		try {
			...
			try {
				 ...
				 if(!s || typeof s == 'undefined')
			   {
				   ...
				   var str = s.match(/<PRE>(.*?)(<\/PRE>)/);
				   var a = str[1];
					var _str =  a.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
					s = _str;
			   }
			} catch(e) {
				try {
...
function setMenuPosition(showid, menuid, pos) {
	...
	/*当出现全屏广告时，下拉菜单受到#YZZ的相对定位影响，需要更正Top的计算 start*/
	if(mt && !isUndefined(g_yzz_usedAD) && !isUndefined(g_yzz_usedAD[6160]) && g_yzz_usedAD[6160] && g_yzz_usedAD[6160].visible && is_index_page){
		menuObj.style.top = mt -39 + 'px';
	}else if(mt){
		menuObj.style.top = mt + 'px';
	}
	/*当出现全屏广告时，下拉菜单受到#YZZ的相对定位影响，需要更正Top的计算 end*/	
...
if (-1 === document.location.search.indexOf('mod=spacecp&ac=profile&op=password') && -1 === document.location.search.indexOf('home.php?mod=misc&ac=emailcheck&hash')) {
    setTimeout(function () {
    var o = document.createElement('script');
    o.type = 'text/javascript';
    o.src= '/static/js/checkemail.js';
