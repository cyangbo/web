...
function _showForummenu(fid) {
	if(!$('fjump_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> 请稍候...</div>';
		$('append_parent').appendChild(menu);
		ajaxget('forum.php?mod=ajax&action=forumjump&yid='+fid, 'fjump_menu', 'ajaxwaitid');
	}
...
function _showForummenu(fid)
{
	//edit.qiezi.20131030 增加二级fid判断
	fid = !fid ? 0 : fid;
	var s = '';
	if(fid)
	{
		var fid_arr = fid.split('_');
		if(fid_arr[0] && fid_arr[1]) 
		{
			s = fid_arr[1] ;
			fid = fid_arr[0];
		}
	...
	if(!$('fjump'+s+'_menu')) {
		menu = document.createElement('div');
		menu.id = 'fjump'+s+'_menu';
		menu.style.display = 'none';
		menu.className = 'p_pop';
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> 请稍候...</div>';
		$('append_parent').appendChild(menu);
		ajaxget('forum.php?mod=ajax&action=forumjump&yid='+fid, 'fjump'+s+'_menu', 'ajaxwaitid');
	}
	showMenu({'ctrlid':'fjump'+s,'ctrlclass':'a','duration':2});
}